/*
 ╔══════════════════════════════════════════════════════════════════════════════╗
 ║  APEX-KV  v2.0  —  Production-Grade Distributed Key-Value Store            ║
 ║  Single-file · C++20 · Linux                                                ║
 ║                                                                              ║
 ║  Senior engineer audit & fixes vs v1:                                       ║
 ║                                                                              ║
 ║  CORRECTNESS                                                                 ║
 ║   ✓ Raft: peer identified by node_id in every RPC (not by fd guess)        ║
 ║   ✓ Raft: WAL with CRC32 + fdatasync before ACKing AppendEntries           ║
 ║   ✓ Raft: send via eventfd+queue so net_thread owns ALL fd writes          ║
 ║   ✓ Raft: AppendEntries resp correctly credits the right peer              ║
 ║   ✓ Gossip: UDP ACK actually sent (was built & dropped in v1)              ║
 ║   ✓ No silent failures: Result<T> propagated, every error logged           ║
 ║                                                                              ║
 ║  SAFETY                                                                      ║
 ║   ✓ Single thread owns all fd I/O (no dangling-pointer race on conn table) ║
 ║   ✓ RAII file descriptors via ScopedFd                                     ║
 ║   ✓ Every atomic op has a comment explaining the required ordering         ║
 ║                                                                              ║
 ║  OBSERVABILITY                                                               ║
 ║   ✓ Leveled logger (DEBUG/INFO/WARN/ERROR) with timestamps, node-id tag   ║
 ║   ✓ Metrics: per-op counters + exponential-bucket latency histograms       ║
 ║   ✓ /metrics endpoint: human-readable stats dump                           ║
 ║                                                                              ║
 ║  PERFORMANCE                                                                 ║
 ║   ✓ Benchmark client pipelines N requests in-flight before reading back    ║
 ║   ✓ Gossip UDP socket is non-blocking with proper EAGAIN handling          ║
 ║                                                                              ║
 ║  TESTABILITY                                                                 ║
 ║   ✓ --test flag: unit tests for HashMap, WAL, Ring, Raft vote logic        ║
 ║                                                                              ║
 ║  Build:  g++ -O3 -march=native -std=c++20 -pthread apex_kv.cpp -o kv      ║
 ║  Test:   ./kv --test                                                        ║
 ║  Node:   ./kv --id 1 --port 7001 --peers 127.0.0.1:7002,127.0.0.1:7003   ║
 ║  Client: ./kv --client 127.0.0.1:7001                                      ║
 ║  Bench:  ./kv --bench 127.0.0.1:7001 --threads 8 --ops 100000             ║
 ╚══════════════════════════════════════════════════════════════════════════════╝
*/
